...
- Layout per connector and account under `<config_dir>/connector-inbox/<connector>/<account>/`:
  ...
  - `dedupe.idx`: bounded rolling index of recently seen provider ids, consulted before append; a duplicate delivery is acknowledged with 200 but not appended. Entries are evicted by age against a 24 hour window, with a 10000-entry size cap as a safety valve. Two limits are worth stating plainly. First, the size cap binds long before the age window on a busy account: at the 600-per-minute accept cap the index turns over in roughly 17 minutes, so the effective replay-protection window there is minutes, not the advertised 24 hours. The age window is what a normal-volume account actually gets. Second, each entry stores a timestamp alongside the id digest; index lines written by a pre-timestamp build carry no timestamp column and are dropped on first read, so upgrading opens a one-time replay window per account until the index refills. Both are bounded and self-healing, and neither affects signature verification, which is what actually authenticates a delivery.
