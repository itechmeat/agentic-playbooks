...
> Mirror rule: CLAUDE.md and AGENTS.md must stay in sync. Any change to the
> shared guidance in one file must be mirrored in the other in the same change.
> Only the "Claude-specific" section below is exempt (it has no AGENTS.md twin).
...
- Navigate the code through codegraph (symbol/edge index) rather than ad-hoc
  grep where possible.

## Claude-specific

- This project has a CodeGraph MCP server (`codegraph_*` tools): prefer it for
  structural questions (definitions, callers, impact) over grep.
- Commit messages end with the Co-Authored-By trailer for the acting model;
  never add a visible AI-authorship marker to public prose.
