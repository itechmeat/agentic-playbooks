...
/// re-stat'ed and reloaded when its mtime or length changes. On the ordinary
/// request path (every request, including exempt and cookie-authenticated
/// ones) the check is throttled to once per [`KEY_RELOAD_INTERVAL_MS`], so a
/// busy server pays one `stat` per minute there. A presented bearer key is
/// checked against the file's current stamp on every attempt regardless of
/// that throttle (see [`AuthState::verify_key_with_reload`]): a key that was
/// just revoked is still sitting in the in-memory set and would otherwise
/// verify successfully against stale data, so only a check on every bearer
/// attempt (not just on failure) makes revocation take effect on the very
/// next request without a restart. Bearer checks are comparatively rare next
/// to total traffic (session cookies never take this path), so the extra
/// `stat` per attempt is one syscall, not a hot-path cost; the parse-and-swap
/// only runs when the stamp actually changed.
...
impl AuthState {
    ...
    /// Force-checks the key file's stamp, then verifies. A key that was just
    /// revoked can still be sitting in the in-memory set (it has not failed
    /// verification yet, so nothing would otherwise trigger a reload), so a
    /// "verify first, force-reload only on failure" order can never reject a
    /// stale-but-still-cached key: the very case revocation needs to hit. A
    /// bearer credential is comparatively rare next to every other request
    /// (session cookies never take this path), so forcing a `stat` here on
    /// every bearer attempt is one syscall per credential check, not per
    /// request; the parse-and-replace cost only applies when the stamp
    /// actually changed. That is what makes issuing a first key or revoking a
    /// compromised one take effect on the very next request rather than on
    /// the next restart, or up to a minute later.
