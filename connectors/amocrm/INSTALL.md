...
- that eight functions are marked `EFFECTFUL, IRREVERSIBLE` (`decline_unsorted`, `delete_pipeline`, `delete_status`, `delete_custom_field`, `delete_transaction`, `set_customers_mode`, `enable_products`, `unsubscribe_webhook`) and belong only in a grant that specifically needs them; six cannot be undone through the API at all, while `set_customers_mode` and `unsubscribe_webhook` can be redone rather than undone (the mode switched again, the webhook resubscribed) and are on the list for their account-wide reach;
